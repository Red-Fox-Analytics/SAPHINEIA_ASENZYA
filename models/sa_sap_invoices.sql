...
(
	SELECT case when "Ownership" is null then 'Not Coded' else "Ownership" end "CUSTOMER_(PARENT)" 
			...
			, case when "year" is null then 'Not Coded' else "year" end "YEAR"
			, case when "Inv No" is null then 'Not Coded' else "Inv No" end "INV_NO" , "Day of Week" "DAY_OF_WEEK"
			, case when "item_wgt" is null then 'Not Coded' else "item_wgt" end "ITEM_WEIGHT", "Table Names-1"  "TABLE_NAMES_1"
			...
			, case when "Cust No" is null then 'Not Coded' else "Cust No" end "CUSTOMER_(CHILD)_ID", "Recipe ID" "RECIPE_ID" 
